...
int main(int argc, char *argv[]) {
	...
	Cuda_Parameter_Initialization(order, core_kernel, core_dimen, dimen,
			...
			&parameter_b_host_to_device, &intermediate_variables_device,
	...
	for (int i = 0; i < iter_number; i++) {
		...
		Update_Parameter_A(order, dimen, core_kernel, core_dimen,
		...
		Update_Parameter_B_Batch(order, dimen, core_kernel, core_dimen,
				parameter_a_device, parameter_b_device, nnz_train,
				value_train_host_to_device, index_train_host_to_device,
				learn_rate_b, lambda_b, intermediate_variables_device);
