...
void Update_Parameter_B_Batch(const int order, int *dimen,
	...
	if (order == 3) {
		Update_Parameter_B_Batch_3(order, dimen, core_kernel, core_dimen, nnz,
				...
				learn_rate_b, lambda_b, intermediate_variables);
	} else if (order == 4) {
...
__global__ void RMSE_AND_MAE_CSF_Order_3(const int order,
...
void GET_RMSE_AND_MAE_CSF_Order_3(const int order, const int core_kernel,
	...
	RMSE_AND_MAE_CSF_Order_3<<<grid_size, block_size>>>(order, 0, core_kernel,
