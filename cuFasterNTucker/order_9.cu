...
__global__ void Update_Parameter_A_SGD_Order_9(const int order,
	...
	type_of_data intermediate_variables_shared_7[register_size];
	type_of_data parameter_b_shared[register_size];

#pragma unroll
	for (int kernel_index = 0; kernel_index < register_size; kernel_index++) {
		parameter_b_shared[kernel_index] = parameter_b[(update_order + 8)
...
#pragma unroll
									...
									for (int order_index_8 = start_7;
										...
										atomicAdd(
												&a_grad_up[(update_order + 8)
														% order][index_8
														+ lane_id],
												value_train_device[update_order][order_index_8]
														* gs);
										atomicAdd(
												&a_grad_down[(update_order + 8)
														% order][index_8
														+ lane_id],
												p_a_gs * gs
														+ lambda_a
...
void Update_Parameter_A_9(const int order, int *dimen, const int core_kernel,
...
void Update_Parameter_B_Batch_9(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		Update_Parameter_B_CSF_Gradient_Order_9
