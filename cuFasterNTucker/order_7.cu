...
#pragma unroll
	for (int kernel_index = 0; kernel_index < register_size; kernel_index++) {
		parameter_b_shared[kernel_index] = parameter_b[(update_order + 6)
...
#pragma unroll
							...
							for (int order_index_6 = start_5;
								...
								atomicAdd(
										&a_grad_up[(update_order + 6) % order][index_6
												+ lane_id],
										value_train_device[update_order][order_index_6]
												* gs);
								atomicAdd(
										&a_grad_down[(update_order + 6) % order][index_6
												+ lane_id],
										p_a_gs * gs
												+ lambda_a
...
void Update_Parameter_A_7(const int order, int *dimen, const int core_kernel,
...
void Update_Parameter_B_Batch_7(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		Update_Parameter_B_CSF_Gradient_Order_7
