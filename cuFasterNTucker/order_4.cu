...
#pragma unroll
	for (int kernel_index = 0; kernel_index < register_size; kernel_index++) {
		parameter_b_shared[kernel_index] = parameter_b[(update_order + 3)
...
#pragma unroll
				for (int kernel_index = 0; kernel_index < register_size;
					...
					gs_temp *= intermediate_variables_shared_2[kernel_index];
					gs += gs_temp;
				}
				...
				for (int order_index_3 = start_2; order_index_3 < end_2;
					...
					atomicAdd(
							&a_grad_up[(update_order + 3) % order][index_3
									+ lane_id],
							value_train_device[update_order][order_index_3]
									* gs);
					atomicAdd(
							&a_grad_down[(update_order + 3) % order][index_3
									+ lane_id],
							p_a_gs * gs
									+ lambda_a
											* parameter_a[(update_order + 3)
													% order][index_3 + lane_id]);
				}
			}
		}
	}
...
#pragma unroll
					for (int core_kernel_index = 0;
						...
						b_gard_up_temp[core_kernel_index] +=
								value_train_device[update_order][order_index_3]
										* aqs;
						b_gard_down_temp[core_kernel_index] += p_a_gs * aqs;
...
void Update_Parameter_A_4(const int order, int *dimen, const int core_kernel,
...
void Update_Parameter_B_Batch_4(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		Update_Parameter_B_CSF_Gradient_Order_4
