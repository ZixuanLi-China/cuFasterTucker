...
#pragma unroll
			...
			for (int order_index_2 = start_1; order_index_2 < end_1;
				...
				atomicAdd(
						&a_grad_up[(update_order + 2) % order][index_2 + lane_id],
						value_train_device[update_order][order_index_2] * gs);
				atomicAdd(
						&a_grad_down[(update_order + 2) % order][index_2
								+ lane_id],
						p_a_gs * gs
								+ lambda_a
										* parameter_a[(update_order + 2) % order][index_2
...
void Update_Parameter_B_Batch_3(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		core_kernel / (block_size / core_dimen) + 1, block_size>>>(
				...
				parameter_b, lambda_b, update_order);
		cudaDeviceSynchronize();

		Update_Parameter_B<<< core_kernel / (block_size / core_dimen) + 1,
		block_size>>>(update_order, core_kernel, core_dimen, parameter_b,
				b_grad_up, b_grad_down);
