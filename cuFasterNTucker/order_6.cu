...
#pragma unroll
	for (int kernel_index = 0; kernel_index < register_size; kernel_index++) {
		parameter_b_shared[kernel_index] = parameter_b[(update_order + 5)
...
#pragma unroll
						...
						for (int order_index_5 = start_4; order_index_5 < end_4;
							...
							atomicAdd(
									&a_grad_up[(update_order + 5) % order][index_5
											+ lane_id],
									value_train_device[update_order][order_index_5]
											* gs);
							atomicAdd(
									&a_grad_down[(update_order + 5) % order][index_5
											+ lane_id],
									p_a_gs * gs
											+ lambda_a
...
#pragma unroll
							for (int core_kernel_index = 0;
								...
								b_gard_up_temp[core_kernel_index] +=
										value_train_device[update_order][order_index_5]
												* aqs;
								b_gard_down_temp[core_kernel_index] += p_a_gs
										* aqs;
...
void Update_Parameter_A_6(const int order, int *dimen, const int core_kernel,
...
void Update_Parameter_B_Batch_6(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		Update_Parameter_B_CSF_Gradient_Order_6
