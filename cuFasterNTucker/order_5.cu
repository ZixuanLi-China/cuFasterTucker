...
#pragma unroll
	for (int kernel_index = 0; kernel_index < register_size; kernel_index++) {
		parameter_b_shared[kernel_index] = parameter_b[(update_order + 4)
...
#pragma unroll
					...
					for (int order_index_4 = start_3; order_index_4 < end_3;
						...
						atomicAdd(
								&a_grad_up[(update_order + 4) % order][index_4
										+ lane_id],
								value_train_device[update_order][order_index_4]
										* gs);
						atomicAdd(
								&a_grad_down[(update_order + 4) % order][index_4
										+ lane_id],
								p_a_gs * gs
										+ lambda_a
...
void Update_Parameter_B_Batch_5(const int order, int *dimen,
	...
	for (int update_order = 0; update_order < order; update_order++) {
		...
		Update_Parameter_B_CSF_Gradient_Order_5
