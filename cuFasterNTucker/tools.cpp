...
void Cuda_Parameter_Initialization(int order, int core_kernel, int core_length,
	...
	*a_grad_up_host_to_device = (type_of_data**) malloc(
			sizeof(type_of_data*) * order);
	*a_grad_down_host_to_device = (type_of_data**) malloc(
			sizeof(type_of_data*) * order);
	...
	*value_train_host_to_device = (type_of_data**) malloc(
	...
	cudaMalloc((void**) &(*a_grad_up), sizeof(type_of_data*) * order);
	cudaMalloc((void**) &(*a_grad_down), sizeof(type_of_data*) * order);
	...
	*parameter_b_host_to_device = (type_of_data**) malloc(
	...
	for (int i = 0; i < order; i++) {
		...
		type_of_data *temp_up;
		cudaMalloc((void**) &temp_up,
				dimen_host[i] * core_dimen * sizeof(type_of_data));
		cudaMemset(temp_up, 0,
				dimen_host[i] * core_dimen * sizeof(type_of_data));
		(*a_grad_up_host_to_device)[i] = temp_up;

		type_of_data *temp_down;
		cudaMalloc((void**) &temp_down,
				dimen_host[i] * core_dimen * sizeof(type_of_data));
		cudaMemset(temp_down, 0,
				dimen_host[i] * core_dimen * sizeof(type_of_data));
		(*a_grad_down_host_to_device)[i] = temp_down;
	...
	*intermediate_variables_host_to_device = (type_of_data**) malloc(
	...
	cudaMemcpy(*a_grad_up, *a_grad_up_host_to_device,
			sizeof(type_of_data*) * order, cudaMemcpyHostToDevice);
	cudaMemcpy(*a_grad_down, *a_grad_down_host_to_device,
			sizeof(type_of_data*) * order, cudaMemcpyHostToDevice);
}
